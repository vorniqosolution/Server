...
const {
  uploadImageToS3,
  deleteImageFromS3,
} = require("../service/imageUploadService");
...
const parseRequirements = (reqData) => {
  ...
  try {
    ...
    throw new Error(
      "Invalid format for inventoryRequirements. Must be valid JSON string."
    );
  }
};
  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      ...
      const uploadPromises = req.files.map((file) =>
        uploadImageToS3(file, "images/decor")
      );
    ...
    if (inventoryRequirements) {
      ...
      if (!Array.isArray(parsedRequirements)) {
        return res
          .status(400)
          .json({ message: "Inventory requirements must be a list (array)." });
      }
      ...
      for (const reqItem of parsedRequirements) {
        if (!reqItem.item || !reqItem.quantity) {
          return res
            .status(400)
            .json({ message: "Ingredients need Item ID and Quantity." });
        }
      }
    }
...
exports.updatePackage = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, description, price, inventoryRequirements, keepOldImages } =
      req.body;
    ...
    const updatedPackage = await DecorPackage.findByIdAndUpdate(id, updates, {
      new: true,
      runValidators: true,
...
exports.getPackages = async (req, res) => {
  try {
    const packages = await DecorPackage.find()
      .populate("inventoryRequirements.item", "name quantityOnHand")
      .lean(); // use lean to get plain JS objects

    const formattedPackages = packages.map((pkg) => ({
      ...pkg,
      inventoryRequirements: pkg.inventoryRequirements.map((reqItem) => ({
        item: reqItem.item ? reqItem.item.name : null,
...
exports.completeAndBillOrder = async (req, res) => {
  try {
    ...
    if (invoice) {
      ...
      const decorPkg = await DecorPackage.findById(order.package).populate(
        "inventoryRequirements.item"
      );
      ...
      return res.status(404).json({
        message:
          "Active Invoice not found. Guest is checked in but has no pending invoice.",
      });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Stock deducted & Invoice updated." });
  } catch (err) {
